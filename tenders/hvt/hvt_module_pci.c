...
static void hvt_x86_setup_dma_pagetable(struct hvt *hvt, struct mft *mft)
{
    ...
    for (uint64_t paddr = HVT_DMA_BASE; paddr < HVT_DMA_BASE + size; paddr += HUGE_PAGE_SIZE, pde++)
 ...
 *
 * PML4:
 * 0x0 -> PDPTE (main memory)
 * 0x1 -> PDPTE_PCI (PCIe regions)
 * 0x2 -> PDPTE_DMA (DMA-ready memory)
 *
 * PDPTE_DMA:
 * 0x0 -> PDE_DMA
 * 
 * PDE_DMA:
 * 0x0 -> HVT_DMA_BASE
 * 0x1 -> HVT_DMA_BASE + 2 MiB
 * ...
 * (mft->dma_size - 1) / 2 MiB -> HVT_DMA_BASE + ((mft->dma_size - 1) * 2 MiB)
 *
 * PDPTE:
 * 0x0 -> PDE
 *
 ...
 * 0x3 -> 6 MiB, PS
 * ...
 * (mem_size - 1) / 2 MiB -> mem_size - 2 MiB, PS
 *
 * PT0E:
 * 0x0 -> not present
 * 0x1 -> not present
 * 0x2 -> not present
 * ...
 * (X86_PT0_MAP_START - 1) / 4 KiB -> not present
 *
 * X86_PT0_MAP_START / 4 KiB -> X86_PT0MAP_START, RO
 * ...
 * (X86_GUEST_MIN_BASE - 1) / 4 KiB -> X86_GUEST_MIN_BASE - 2 MiB, RO
 *
 * X86_GUEST_MIN_BASE / 4 KiB -> X86_GUEST_MIN_BASE
 * ...
 * (2 MiB - 4 KiB) / 2 MiB -> 2 MiB - 4 KiB
 */

/*
 * TODO this works but is super-wonky
 * we always map 2 MiB per region into the unikernel, even if the region is smaller.
 * we only support 1 pci device.
 */
static void hvt_x86_setup_pci_device_pagetable(struct hvt *hvt, struct mft_entry *e)
{
...
#define setup_region_pagetable(n) \
    do { \
        if (pci.map_bar ## n) { \
            ...
            pde[n] = HVT_REGION(pci.device_index, (n)) | (X86_PDPT_P | X86_PDPT_RW | X86_PDPT_PS); \
        } \
    } while (0)
...
static int setup(struct hvt *hvt, struct mft *mft)
{
    if (!module_in_use && mft->dma_size != 0)
        warn("DMA memory requested but no PCI devices mapped, ignoring DMA request");
    ...
    if (mft->dma_size > 0) {
        if (mft->dma_size & (HUGE_PAGE_SIZE - 1)) {
            mft->dma_size = ((mft->dma_size >> HUGE_PAGE_BITS) + 1) << HUGE_PAGE_BITS;
        }
...
#define map_region(n) \
    do { \
        if (e->u.pci_basic.map_bar ## n) { \
            size_t size; \
            void *buffer = pci_map_region(e->b.hostfd, VFIO_PCI_BAR ## n ## _REGION_INDEX, &size); \
            if (buffer == MAP_FAILED) { \
            ...
            if (ioctl(hvt->b->vmfd, KVM_SET_USER_MEMORY_REGION, &region) == -1) \
                err(1, "KVM: ioctl (SET_USER_MEMORY_REGION) for BAR" #n " failed"); \
